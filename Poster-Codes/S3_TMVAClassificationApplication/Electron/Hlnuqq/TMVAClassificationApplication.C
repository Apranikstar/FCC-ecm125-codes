...
void TMVAClassificationApplication( TString myMethodList = "" )
{
   ...
   //
   TFile *input(0);
   TString fname = "Electron_DataBase/Electron_wzp6_ee_qq_ecm125.root";
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      /// MY CODE !!!!
      if(Use["BDT"]) {
        if ( (reader -> EvaluateMVA("BDT method")) >     0.1   )    AS_cut[1]++; // -0.35 for t_br and -0.40 for t_bi (test both for -0.1)
      }
